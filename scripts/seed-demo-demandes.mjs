...
const DEMO_REQUESTS = [
  ...
  },
  {
    ...
    banAddressId: "62041_2020_00008",
    latitude: 50.290812,
    longitude: 2.777164,
    description:
      "Maison. Je souhaite des travaux d'électricité : Remplacement tableau électrique (logement, conformité). Mise aux normes d'une maison des années 70, compteur monophasé, et ajout de 6 prises USB dans la cuisine et le bureau.\n\nMerci de me recontacter pour un devis. Je préciserai les détails lors de l'échange.",
    workOptionId: "4321a-tableau",
    pricingTier: "eleve",
    nafCodes: ["43.21A"],
    auctionDurationHours: 720,
    maxContactArtisans: 5,
...
async function main() {
  ...
  for (const demo of DEMO_REQUESTS) {
    ...
    const auctionDurationHours =
      demo.auctionDurationHours ??
      (demo.auctionDurationDays ? demo.auctionDurationDays * 24 : 720);
    const request = {
      id,
      ...
      banAddressId: demo.banAddressId,
      latitude: demo.latitude,
      longitude: demo.longitude,
      addressVerifiedAt: demo.banAddressId ? now : undefined,
      category: demo.category,
      description: demo.description,
      nafCodes: demo.nafCodes,
      pricingTier: demo.pricingTier,
      workOptionId: demo.workOptionId,
      startPrice: demo.startPrice,
      startPriceMode:
        demo.startPrice != null ? "client" : "unspecified",
      auctionDurationHours,
      auctionDurationDays: Math.max(1, Math.round(auctionDurationHours / 24)),
      maxContactArtisans: demo.maxContactArtisans ?? 5,
      preferEstablishedCompany: demo.preferEstablishedCompany,
      minGoogleRating: demo.minGoogleRating,
